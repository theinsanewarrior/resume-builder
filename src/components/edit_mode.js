...
class EditMode extends Component{
    ...
    render(){
      return(
        ...
        />
        {/* <h4>Projects</h4>
        <button onClick={this.props.projectsHandler}>ADD</button>
        {this.props.projectDetails.map((items,step)=>{
            const id=items.id
            return(
                <ProjectDetails 
                    key={id}
                    ...
                    titlHandler={(ev)=>{this.props.titlHandler(ev,id)}}
                    describeHandler={(ev)=>{this.props.describeHandler(ev,id)}}
                />
            )
        })} */}
